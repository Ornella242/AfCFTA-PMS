        ...
        <div class="row justify-content-center">
            <div class="col-12">
                <div class="row align-items-center mb-4">
                    <div class="col">
                        <h2 class="h3 mb-0 page-title text-maroon">Pending task archivation requests</h2>
                    </div>
                    @if(session('success'))
                ...
                @foreach($requests as $req)
                    <div class="card mb-3">
                        <div class="card-body">
                            <h5 class="card-title">{{ $req->task->title }}</h5>
                            <p><strong>Requested by:</strong> {{ $req->requester->firstname }} {{ $req->requester->lastname }}</p>

                            @if(!$req->approved)
                                <form action="{{ route('taskArchiveRequests.approve', $req->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('PATCH')
                                    <button class="btn btn-success btn-sm">Approve Archivation</button>
                                </form>

                                <form action="{{ route('taskArchiveRequests.decline', $req->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
